...
T = 총 투자금 / 시드
- T=1: 1턴 (시드 1회 투입)
- T=3: 3턴 (시드 3회 투입, 물타기 2회)
- T=5: 5턴 (Max Tier 도달 → 손절)

### PointLoc - 동적 매수/매도 기준
**T값에 따라 매일 매수/매도 기준가가 자동으로 변경됩니다.**

```python
point = get_point_loc(div_value, max_tier, t, point_loc)
매수가 = 평단가 × (1 + point)
매도가 = 평단가 × (1 + point)
```

예시 (PointLoc.P2_3, div=20%, max_tier=5):

| T턴  | point | 매수/매도 기준 | 전략      |
|------|-------|----------------|-----------|
| 1    | +14%  | 평단가 ±14%    | 넓은 밴드 |
| 2    | +8%   | 평단가 ±8%     | 밴드 축소 |
| 3.33 | 0%    | 평단가 (본전)  | 본전 전략 |
...
**핵심**: 턴이 진행될수록 매수/매도 기준이 평단가로 수렴 → 일정 턴 이후 손절 모드 전환

### 동적 시드 - 점진적 자금 투입
- 초기 시드: $100 (작게 시작)
- 목표 시드: $1000 (dynamic_seed_max)
